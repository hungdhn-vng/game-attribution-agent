...
- Self-extension (the agent promoting its own scratch scripts into tools) — noted as a future direction only.
...
└── skills/gaa/
    ...
    └── references/
        ...
        └── admin.md              # config/profile commands
```

`SKILL.md` decision guide: fresh question → `analyze`; follow-up about an existing run → a primitive; no primitive fits → Tier-3 code per `adhoc.md`; unsure what runs exist → `gaa jobs`; never invent a run id. Reference files hold exact command lines, loaded only when doing that task — keeping the orchestrator's context small and its rails tight.
...
- Tool promotion: an admin asks the agent to graduate a proven scratch script into a named primitive + skill reference — the system growing its own toolbox.
